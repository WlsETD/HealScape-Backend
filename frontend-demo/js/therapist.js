(function therapistApp() {
  ...
  async function loadPatients() {
    try {
        ...
        const newList = list.map(function(p) {
            const existing = PATIENTS.find(function(old) { return old.id === p.id; });
            return {
                ...p,
        ...
        const syncPromises = list.map(function(p, index) {
            return new Promise(function(resolve) {
                setTimeout(async function() {
                    try {
                        const fhir = await window.healscapeApi.getPatientData(p.id);
                        if (fhir.history && fhir.history.length > 0) {
                            const latestRom = fhir.history.find(function(h) { return h.type === 'rom'; });
                            const latestBp = fhir.history.find(function(h) { return h.type === 'bp' || h.type === 'Blood Pressure'; });
                            ...
                            let riskLevel = 'low'; // 'low', 'mid', 'high'
                            let alertMsg = '穩定恢復: ' + fhir.history[0].date;
                            
                            const sys = latestBp ? latestBp.value : 0;
                            const dia = latestBp ? latestBp.reps : 0;

                            // 高風險：符合高血壓定義 (140/90) 或 ROM 過低
                            if ((sys >= 140 || dia >= 90) || (PATIENTS[index].rom > 0 && PATIENTS[index].rom < 50) || p.id === '104') {
                                riskLevel = 'high';
                                alertMsg = (sys >= 140 || dia >= 90) ? '警報：血壓達高血壓標準！' : '警報：活動度急遽下降！';
                            } 
                            // 中風險：高血壓前期 (120-139 或 80-89)
                            else if (sys >= 120 || dia >= 80) {
                                riskLevel = 'mid';
                                alertMsg = '提示：血壓處於高血壓前期';
                            }

                            PATIENTS[index].risk = riskLevel;
                            ...
                            const romHistory = fhir.history.filter(function(h) { return h.type === 'rom' && (h.rom !== undefined || h.value !== undefined); });
                            if (romHistory.length > 0) {
                              // 取得最近 7 筆數據，並反轉使其由舊到新
                              const last7 = romHistory.slice(0, 7).reverse();
                              PATIENTS[index].historyData = last7.map(function(h) { return h.rom !== undefined ? h.rom : h.value; });
                              PATIENTS[index].historyLabels = last7.map(function(h) {
  ...
  async function selectPatient(id) {
    ...
    const p = PATIENTS.find(function(pt) { return pt.id === id; });
    render(); 

    const syncData = async function() {
        try {
            const [balance, ledger, fhir] = await Promise.all([
                window.healscapeApi.getHEALBalance(id).catch(function(e) { return 0; }),
                window.healscapeApi.getLedger(id).catch(function(e) { return []; }),
                window.healscapeApi.getPatientData(id).catch(function(e) { return { history: [] }; })
            ]);
            ...
            const currentDataSnapshot = JSON.stringify({ 
                balance: balance, 
                ledger: ledger, 
                fhirLength: (fhir.history ? fhir.history.length : 0), 
                lastRom: (fhir.history ? fhir.history.find(function(h) { return h.type === 'rom'; }) : null)
            });
            ...
            // 更新圖表數據與日期標籤 (僅針對 ROM)
            const filteredRom = fhir.history.filter(function(h) { return h.type === 'rom' && (h.rom !== undefined || h.value !== undefined); });
            if (filteredRom.length > 0) {
              const last7 = filteredRom.slice(0, 7).reverse();
              p.historyData = last7.map(function(h) { return h.rom !== undefined ? h.rom : h.value; });
              p.historyLabels = last7.map(function(h) {
                  if (!h.date || h.date === 'N/A') return '--/--';
                  return h.date.split('-').slice(1).join('/'); // 取 月/日
              });
            ...
            state.selectedPatientData = { balance: balance, ledger: ledger, fhir: fhir };
            render();
  ...
  function render() {
    ...
    const headerHtml = `
      ...
      <div class="grid grid-cols-3 gap-3 relative z-10">
        ...
        <button data-act="filter-high" class="bg-white/5 border ${state.filterRisk === 'high' ? 'border-red-500/50 bg-red-500/5' : 'border-white/10'} p-3 rounded-2xl text-center hover:bg-white/10 transition-all active:scale-95">
          ...
          <div class="text-xl font-black text-red-400">${PATIENTS.filter(function(p){return p.risk==='high'}).length}</div>
        </button>
    ...
    const searchHtml = `
      ...
      <select id="risk-filter" class="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl px-3 py-3 text-sm focus:outline-none font-bold">
        ...
        <option value="mid" ${state.filterRisk==='mid'?'selected':''}>中風險</option>
    ...
    // 更新列表
    const listHtml = filtered.map(function(p) { return renderPatientCard(p); }).join('');
    safeSetInnerHTML(document.getElementById('patient-list-inner'), listHtml);

    // 更新詳細資料與 Modal
    const detailsContainer = document.getElementById('details-container');
    const newDetailsHtml = state.selectedId ? renderPatientDetails(PATIENTS.find(function(p){return p.id===state.selectedId})) : '';
    ...
    if (detailsScrollEl && state.selectedId && detailsContainer.dataset.renderedId === state.selectedId) {
        // 僅更新特定數值以防止閃爍
        const p = PATIENTS.find(function(pt) { return pt.id === state.selectedId; });
        const balEl = detailsContainer.querySelector('.balance-val');
        const displayBal = state.selectedPatientData ? state.selectedPatientData.balance : p.healBal;
        if (balEl) balEl.innerText = Math.floor(displayBal);
    ...
    safeSetInnerHTML(document.getElementById('modal-container'), state.showPrescribeModal ? renderPrescribeModal(PATIENTS.find(function(p){return p.id===state.selectedId})) : '');
    ...
    if (state.selectedId) {
        ...
        const p = PATIENTS.find(function(p) { return p.id === state.selectedId; });
        setTimeout(function() { initChart(p); }, 100);
    }
  }
        ...
        <div class="flex justify-between items-center">
          <div class="flex gap-4 items-center">
            ...
            <div>
              ...
              <p class="text-[10px] ${getRiskClass(p.risk)} mt-0.5 flex items-center gap-1">
                <span class="w-1 h-1 rounded-full ${getPulseClass(p.risk)}"></span>
                ${p.alert}
              </p>
            </div>
          </div>
          <div class="flex items-center gap-4">
            <div class="text-right hidden sm:block">
              ...
              <div class="text-xl font-black ${getBpColor(p.bp)}">${p.bp || '--/--'}</div>
  ...
  // 輔助函數：取得血壓顏色與風險樣式
  const getBpColor = function(bpStr) {
    if (!bpStr || bpStr === '--/--') return 'text-[var(--text-main)]';
  ...
  const getRiskClass = function(risk) {
    if (risk === 'high') return 'text-red-500 font-bold';
    if (risk === 'mid') return 'text-yellow-500 font-bold';
    return 'text-[var(--text-muted)]';
  };

  const getPulseClass = function(risk) {
    if (risk === 'high') return 'bg-red-500 animate-pulse';
    if (risk === 'mid') return 'bg-yellow-500';
    return 'bg-slate-300';
  };
  ...
  function renderPatientDetails(p) {
    ...
    const couponCount = displayHistory.filter(function(b){ return b.data.type === 'BURN' && b.data.task.indexOf('兌換') !== -1; }).length;
        ...
        <div id="details-scroll-container" class="px-8 flex-1 overflow-y-auto no-scrollbar pb-20">
          ...
          <div class="mb-8">
            ...
            <div id="blockchain-history-container" class="space-y-3 max-h-[300px] overflow-y-auto no-scrollbar pr-1 custom-scroll">
              ${displayHistory.length > 0 ? displayHistory.map(function(b) { return `
  ...
  function attachEvents() {
    ...
    app.addEventListener('click', function(e) {
      ...
      if (act === 'toggle-theme') {
        ...
        const p = PATIENTS.find(function(pt) { return pt.id === state.selectedId; });
        window.showBlockchainProgress('正在為 ' + maskName(p.name) + ' 簽署智能合約處方...', 3000).then(async function() {
            try {
