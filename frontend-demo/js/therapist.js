(function therapistApp() {
        ...
        PATIENTS.forEach((p, idx) => {
          if (p.bp && p.bp.includes('/')) {
            ...
            if (sys >= 140 || dia >= 90) {
              ...
              PATIENTS[idx].alert = '警報：血壓達高血壓標準！';
            } else if (sys >= 120 || dia >= 80) {
              PATIENTS[idx].risk = 'mid';
              PATIENTS[idx].alert = '提示：血壓處於高血壓前期';
            }
          }
        });
                        ...
                        if (fhir.history && fhir.history.length > 0) {
                            ...
                            if (romHistory.length > 0) {
                              ...
                              PATIENTS[index].historyLabels = last7.map(h => {
                                  ...
                                  return h.date.split('-').slice(1).join('/');
                              });
  ...
  const getBpColor = (bpStr) => {
    ...
    if (sys >= 120 || dia >= 80) return 'text-orange-500';
    return 'text-teal-600';
  };

  const getRiskClass = (risk) => {
    if (risk === 'high') return 'text-red-500 font-bold';
    if (risk === 'mid') return 'text-orange-500 font-bold';
    return 'text-[var(--text-muted)]';
  };

  const getPulseClass = (risk) => {
    if (risk === 'high') return 'bg-red-500 animate-pulse';
    if (risk === 'mid') return 'bg-orange-500';
    return 'bg-slate-300';
  };
