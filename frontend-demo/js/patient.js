(function patientApp() {
  ...
  async function init() {
    ...
    // 1. 優先獲取區塊鏈資料 (不依賴外部 FHIR)
    try {
      await window.blockchain.refresh();
      state.web3.healBalance = window.blockchain.getBalance();
      render(); // 先渲染一次以顯示餘額
    } catch(e) { console.error("Blockchain load failed", e); }

    try {
      // 2. 同步資料 (等級、XP、FHIR)
      const [profile, fhirSync] = await Promise.all([
        window.healscapeApi.getProfile(state.patientId),
        window.healscapeApi.syncPatient(state.patientId, sessionStorage.getItem('userName') || 'Patient')
      ]);
      
      // 以後端等級資料為準，更新狀態與 sessionStorage
      state.stats.level = parseInt(profile.level) || 1;
      state.stats.xp = parseInt(profile.xp) || 0;
      state.profile = profile; // 儲存完整個人資料
  ...
  function checkDailyLimits() {
    ...
    if (lastDate !== today) {
      ...
      localStorage.setItem('has_measured_bp_' + state.patientId, 'false');
    }
    ...
    state.hasMeasuredBp = localStorage.getItem('has_measured_bp_' + state.patientId) === 'true';
  }
  ...
  async function redeemDiscount() {
    ...
    if (ok) {
        try {
            ...
            await window.blockchain.burn(amount, "兌換診斷費折扣");
            state.web3.healBalance = window.blockchain.getBalance();
            toast("兌換成功！已折抵 10 元");
            render();
        } catch(e) {
  ...
  async function gainXp(amount) {
    ...
    // 同步到後端 JSON DB
    try {
      await window.healscapeApi.updateProfile(state.patientId, state.stats.level, state.stats.xp);
  ...
  function render() {
    ...
    // 保存捲動位置
    const mainEl = document.getElementById('patient-main-container');
    const mainScroll = mainEl ? mainEl.scrollTop : 0;
    const lastView = app.dataset.currentView;

    const isHome = state.currentView === 'home';
    const xpPct = Math.min((state.stats.xp / state.stats.nextLevelXp) * 100, 100);

    const h = (state.profile && state.profile.height) ? state.profile.height / 100 : 0;
    const w = (state.profile && state.profile.weight) ? state.profile.weight : 0;
    const bmi = (h > 0 && w > 0) ? (w / (h * h)).toFixed(1) : '--';

    app.style.height = "100%";
    app.dataset.currentView = state.currentView;
    ...
    app.innerHTML = `
      <section class="h-full flex flex-col bg-[var(--bg-app)] text-[var(--text-main)] overflow-hidden font-sans">
        <header class="bg-[var(--bg-header)] p-7 pb-5 shrink-0 border-b border-[var(--border-color)] shadow-sm">
          <div class="flex justify-between items-start mb-4">
            ...
            <div class="flex items-center gap-2">
              <button data-act="quick-mock" class="w-10 h-10 bg-teal-500/10 hover:bg-teal-500/20 rounded-xl flex items-center justify-center text-teal-600 transition-colors border border-teal-500/20 animate-pulse" title="測試用：快速模擬上傳 115°">
        ...
        <main id="patient-main-container" class="flex-1 overflow-y-auto no-scrollbar p-5 pt-2">
          ${isHome ? (
    ...
    if (lastView === state.currentView) {
      const newMainEl = document.getElementById('patient-main-container');
      if (newMainEl) newMainEl.scrollTop = mainScroll;
    }
    ...
    return `
      ...
      <div class="bg-white/80 backdrop-blur-xl p-6 rounded-[32px] border border-[var(--border-color)] mb-6 shadow-sm relative overflow-hidden">
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="text-right">
             <div class="text-[9px] font-bold text-teal-600/60 uppercase">連續登入</div>
      ...
      <div class="mb-8">
        ...
        <div class="flex gap-4 overflow-x-auto no-scrollbar pb-2">
          ${state.web3.sbtList.map(function(sbt) { return `
      ...
      <div class="bg-[var(--bg-card)] p-6 rounded-[32px] text-[var(--text-main)] shadow-sm border border-[var(--border-color)] mb-6 relative overflow-hidden">
        ...
        <div class="grid grid-cols-3 gap-3 mb-6">
          ${state.mathQuiz.options.map(function(o) { return `
      ...
      <div class="space-y-4 mb-6">
        ${(function() {
          // 查找處方簽中的設定，若無則使用預設值
          const armPres = state.prescriptions.find(function(p) { return p.task === 'arm'; }) || { reps: 3, difficulty: 90 };
          const gripPres = state.prescriptions.find(function(p) { return p.task === 'grip'; }) || { reps: 3, difficulty: 40 };
        ...
        <button data-act="start-bp" ${state.hasMeasuredBp ? 'disabled' : ''} class="w-full bg-[var(--bg-card)] p-6 rounded-[32px] border border-[var(--border-color)] flex justify-between items-center active:scale-95 transition-all shadow-sm hover:border-red-500/30 disabled:opacity-50 disabled:grayscale">
          <div class="text-left">
            <div class="font-black text-[var(--text-main)] text-lg">每日血壓回報 ${state.hasMeasuredBp ? '(已完成)' : ''}</div>
            <div class="text-[10px] font-bold text-[var(--text-muted)] mt-1 uppercase">目標：每日量測一次</div>
          </div>
          <div class="w-12 h-12 rounded-2xl bg-red-500/5 flex items-center justify-center text-red-600">
      ...
      <div class="rounded-[40px] aspect-[3/4] relative overflow-hidden shadow-2xl ${isReaction ? 'bg-slate-900' : 'bg-black'} border-4 border-[var(--border-color)]" id="game-container">
        ...
        ${isReaction ? `
          <div id="reaction-area" class="absolute inset-0 z-10">
            ${state.reaction.targets.map(function(t) { return `
  ...
  function renderDataView() {
    ...
    const avgROM = state.history.length ? Math.round(state.history.reduce(function(a, b) { return a + (b.rom || 0); }, 0) / state.history.length) : 0;
    ...
    return `
      <div class="space-y-6 text-[var(--text-main)]">
        <div class="bg-[var(--bg-card)] p-6 rounded-3xl shadow-sm border border-[var(--border-color)]">
          <h3 class="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] mb-6 px-1 flex justify-between">
            個人生理數據
            <span class="text-teal-600 font-black">醫護端同步中</span>
          </h3>
          <div class="grid grid-cols-3 gap-3 text-center">
            <div class="p-4 bg-[var(--bg-app)] rounded-2xl border border-[var(--border-color)]">
              <div class="text-[9px] font-bold text-[var(--text-muted)] uppercase mb-1">身高</div>
              <div class="text-xl font-black text-[var(--text-main)]">${(state.profile && state.profile.height) || '--'}<span class="text-[10px] ml-0.5">cm</span></div>
            </div>
            <div class="p-4 bg-[var(--bg-app)] rounded-2xl border border-[var(--border-color)]">
              <div class="text-[9px] font-bold text-[var(--text-muted)] uppercase mb-1">體重</div>
              <div class="text-xl font-black text-[var(--text-main)]">${(state.profile && state.profile.weight) || '--'}<span class="text-[10px] ml-0.5">kg</span></div>
            </div>
            <div class="p-4 bg-[var(--bg-app)] rounded-2xl border border-[var(--border-color)]">
              <div class="text-[9px] font-bold text-[var(--text-muted)] uppercase mb-1">BMI</div>
              <div class="text-xl font-black text-teal-600">${bmi}</div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-[var(--bg-card)] p-6 rounded-3xl shadow-sm border border-[var(--border-color)]">
          <h3 class="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] mb-6 px-1 flex justify-between">
            復健數據概覽
            <span class="text-teal-600 font-black">HAPI FHIR 連線中</span>
          </h3>
          <div class="grid grid-cols-2 gap-4 text-center">
            <div class="p-4 bg-[var(--bg-app)] rounded-2xl border border-[var(--border-color)]">
              <div class="text-[9px] font-bold text-[var(--text-muted)] uppercase mb-1">平均角度 (ROM)</div>
              <div class="text-2xl font-black text-teal-600">${avgROM}°</div>
            </div>
            <div class="p-4 bg-[var(--bg-app)] rounded-2xl border border-[var(--border-color)]">
              <div class="text-[9px] font-bold text-[var(--text-muted)] uppercase mb-1">總紀錄筆數</div>
              <div class="text-2xl font-black text-teal-600">${state.history.length}</div>
            </div>
          </div>
        </div>
        ...
        <!-- FHIR 歷史紀錄 -->
        <div class="bg-[var(--bg-card)] p-6 rounded-3xl shadow-sm border border-[var(--border-color)]">
          <h3 class="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] mb-4 px-1">HAPI FHIR 歷史存證 (Observations)</h3>
          <div class="space-y-3">
            ${state.history.length === 0 ? '<div class="text-center py-4 text-slate-400 text-xs font-bold">尚無 FHIR 數據紀錄</div>' : 
              state.history.map(function(h) { return `
                <div class="flex flex-col gap-2 p-4 bg-[var(--bg-app)] rounded-2xl border border-[var(--border-color)]">
                  <div class="flex justify-between items-center">
                    <div>
                      <div class="text-[10px] font-black uppercase text-teal-600">${h.type === 'rom' ? '肩關節角度 (ROM)' : '抓握訓練'}</div>
                      <div class="text-[8px] font-mono text-[var(--text-muted)] mt-1">${h.date}</div>
                    </div>
                    <div class="text-right">
                      <div class="text-lg font-black">${h.value} <span class="text-[10px] text-slate-400 font-bold">${h.type === 'rom' ? 'deg' : 'kg'}</span></div>
                    </div>
                  </div>
                  <div class="pt-2 border-t border-dashed border-slate-200/50 flex justify-between items-center">
                    <div class="text-[8px] font-mono text-slate-400">FHIR ID: ${h.fhirId || '---'}</div>
                    <a href="https://hapi.fhir.org/baseR4/Observation/${h.fhirId}?_format=json" target="_blank" class="text-[8px] font-black text-teal-600 hover:underline flex items-center gap-1">
                      <i class="fa-solid fa-external-link"></i> HAPI FHIR 存證
                    </a>
                  </div>
                </div>
              `; }).join('')
        ...
        <div class="bg-[var(--bg-card)] p-6 rounded-3xl shadow-sm border border-[var(--border-color)]">
          ...
          <div class="space-y-4">
            ${blockchainHistory.map(function(b) { return `
  ...
  async function startEngine() {
    try {
      ...
      if (state.taskMode === 'arm') {
        ...
        detector = new window.Hands({ locateFile: function(f) { return "https://cdn.jsdelivr.net/npm/@mediapipe/hands/" + f; } });
      ...
      const video = document.getElementById('cam');
      if (video) { video.srcObject = stream; await new Promise(function(r) { video.onloadedmetadata = r; }); await video.play(); }
  ...
  function smoothPose(newKps) {
    ...
    const smoothed = newKps.map(function(kp, i) {
      return {
        ...kp,
  ...
  function processReactionLogic(lm) {
    ...
    if (Math.hypot(hx - tx, hy - ty) < 0.18) {
      ...
      if (state.reaction.score >= 10) { 
        ...
        if (completeBtn) setTimeout(function() { completeBtn.click(); }, 1200);
      }
  ...
  function drawPose(ctx, pts) {
    ...
    const kp = pts.reduce(function(a, c) { return { ...a, [c.name]: c }; }, {});
    ctx.strokeStyle = '#00FF00'; ctx.lineWidth = 4; ctx.lineCap = 'round';
    const pairs = [
  ...
  async function submitPoPW() {
    ...
    try {
      ...
      if (currentTaskCount < 1) {
        reward = 5;
        // 調用後端 mint (現在是向後端提交 PoPW)
        const block = await window.blockchain.mint(reward, taskLabel);
    ...
    } catch (e) { console.error(e); toast("同步失敗"); } finally { state.web3.isVerifying = false; render(); }
  }
  ...
  function attachEvents() {
    ...
    app.addEventListener('click', async function(e) {
      ...
      if (act === 'quick-mock') {
        // 讓數據隨機跳動 (90 ~ 130 度)，產生明顯的趨勢變化
        const mockRom = Math.floor(Math.random() * (130 - 90 + 1)) + 90;
        const mockReps = 3;
        const earnedXp = mockReps * 100;
        
        await window.showBlockchainProgress("正在模擬 AI 辨識並上傳 115° 到 HAPI FHIR...", 2000);
        ...
        try {
            ...
            // 2. 獲得區塊鏈獎勵 (PoPW)
            const block = await window.blockchain.mint(5, "模擬肩關節訓練 (快速上傳)");
            state.web3.lastTxHash = block.hash;
            state.web3.healBalance = window.blockchain.getBalance();
            ...
            toast("模擬成功！上傳角度: " + mockRom + "°");
            render();
        } catch(e) { toast("模擬上傳失敗"); console.error(e); }
        
      } else if (act === 'reset-tasks') {
        ...
        localStorage.removeItem('has_measured_bp_' + state.patientId);
        ...
        modal.innerHTML = `
          <div class="bg-white rounded-[40px] p-8 w-full max-w-sm shadow-2xl animate-in zoom-in duration-300">
            <h3 class="text-2xl font-black text-slate-800 mb-2">血壓量測回報</h3>
            <p class="text-xs text-slate-400 font-bold uppercase tracking-widest mb-6">請輸入您量測到的數值</p>
            
            <div class="space-y-4">
              <div class="flex gap-4">
                <div class="flex-1">
                  <label class="block text-[10px] font-black text-slate-400 uppercase ml-2 mb-1">收縮壓 (高)</label>
                  <input id="bp-sys" type="number" placeholder="120" class="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 px-6 font-black text-xl focus:outline-none focus:border-teal-500 transition-all">
                </div>
                <div class="flex-1">
                  <label class="block text-[10px] font-black text-slate-400 uppercase ml-2 mb-1">舒張壓 (低)</label>
                  <input id="bp-dia" type="number" placeholder="80" class="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 px-6 font-black text-xl focus:outline-none focus:border-teal-500 transition-all">
                </div>
              </div>
              
              <div class="pt-4 flex gap-3">
                <button onclick="document.getElementById('modalWrap').classList.add('hidden')" class="flex-1 py-4 bg-slate-100 text-slate-400 font-black rounded-2xl">取消</button>
                <button id="btn-submit-bp" class="flex-[2] py-4 bg-teal-600 text-white font-black rounded-2xl shadow-lg shadow-teal-100">確認送出</button>
              </div>
            </div>
          </div>
        `;
        modal.classList.remove('hidden');
        document.getElementById('btn-submit-bp').onclick = async function() {
          const sys = document.getElementById('bp-sys').value;
          const dia = document.getElementById('bp-dia').value;
          if (!sys || !dia) return toast("請輸入數值");
          
          modal.classList.add('hidden');
          await window.showBlockchainProgress("正在記錄血壓數據並上傳 FHIR...", 2500);
          ...
          try {
            ...
            // 2. 領取獎勵
            await window.blockchain.mint(10, "每日血壓回報獎勵");
            state.hasMeasuredBp = true;
            localStorage.setItem('has_measured_bp_' + state.patientId, 'true');
            state.web3.healBalance = window.blockchain.getBalance();
            window.showCoinMinted(10);
            
            toast("數據已安全存證至 FHIR 與區塊鏈！");
            render();
          } catch(e) {
            toast("回報失敗，請稍後再試");
            console.error(e);
        ...
        performCheckIn();
      } else if (act === 'finish-result') { await gainXp(state.lastSession.xp); state.taskStep = 'idle'; generateMathQuiz(); render(); }
        ...
        setTimeout(function() { generateMathQuiz(); render(); }, 300);
      }
    });
  }
  ...
  function loadScript(src) { return new Promise(function(r) { if (document.querySelector('script[src="' + src + '"]')) return r(); const s = document.createElement('script'); s.src = src; s.onload = r; document.head.appendChild(s); }); }

  function toast(msg) {
    const w = document.getElementById('toastWrap'); if (!w) return;
    const t = document.createElement('div'); t.className = 'toast'; t.innerText = msg;
    w.appendChild(t); setTimeout(function() { t.remove(); }, 2000);
  }
